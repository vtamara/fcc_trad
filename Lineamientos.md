...
```
*  Ubica el archivo JSON en el directorio ```seeds/challenges``` de tu copia local y editalo añadiendo texto en las entradas "nameES" y "descriptionES" de los desafios no traducidos. Es importante que mantengas bien la sintaxis Javascript, puedes verificarlo instalando node.js y pasando el archivo JSON por el interprete, por ejemplo ```node seeds/challenges/basic-javascript.json```. También podrás verificar sintaxis copiando y pegando el contenido del archivo en http://jsonlint.com.    Si piensas que no puedes continuar deja un comentario en el mismo problema indicándolo para que otra persona pueda continuar donde quedaste . 
